...
public class CustomMatchmakingLobbyController : MonoBehaviourPunCallbacks
{
    ...
    {
        int tempIndex;
        foreach (Transform k in roomsContainer.transform)
        {
    ...
    {
        //loop through each room in room list
    ...
    {
        string roomName = Random.Range(100,999) + role; //string for saving room name
        Debug.Log("Creating room now");
        //roomListings.Clear();
        RoomOptions roomOps = new RoomOptions() { IsVisible = true, IsOpen = true, PlayerTtl=(byte)roomSize };
